...
class Google_OAuth_Controller extends Controller
{
    /**
     * Redirect the user to the Google authentication page.
     *
    ...
    /**
     * Handle Google callback and generate a JWT token.
     *
        ...
        try {
            ...
            $user = User::firstOrCreate(
                ...
                [
                    ...
                    'google_id' => $googleUser->getId(),
                    'profile_picture' => $googleUser->getAvatar(),
                    'role' => 'recipient', // Default role, can be adjusted based on your logic
                    'password' => bcrypt('random_password'), // Default password for social logins
                ]
            );
            ...
            // Return the JWT token and user details to the frontend
            return response()->json([
                'token' => $token,
                'user' => $user,
            ]);
        } catch (\Exception $e) {
            return response()->json(['error' => 'Failed to login with Google'], 401);
        }
    }
}
